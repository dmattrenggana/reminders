...
export interface ReminderData {
  ...
  confirmationTime: bigint // Not used in V5, kept for interface compatibility
}
...
export class ReminderService {
  ...
  async getReminder(reminderId: number): Promise<ReminderData> {
    try {
      ...
      // V5 contract format only (8 fields):
      ...
      if (data.length !== 8) {
        throw new Error(`Unexpected data length: ${data.length}. Expected 8 (V5 contract format)`)
      }
